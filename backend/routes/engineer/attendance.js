...
router.post("/manual/check-in", engineerCheck, upload.single("faceImage"), async (req, res) => {
  try {
    const engineerId = req.user.id;
    ...
    try {
      parsedFaceFeatures = typeof faceFeatures === 'string' 
        ? JSON.parse(faceFeatures) 
        : faceFeatures;
    } catch (e) {
      return res.status(400).json({ error: "Invalid face features JSON format" });
    }
    ...
    if (manualLabourRes.rows.length === 0) {
      return res.status(500).json({ error: "Failed to create manual labour record" });
    }
    ...
    if (existingAttendance.rows.length > 0) {
      ...
      const updateRes = await pool.query(
        `UPDATE attendance 
         ...
         WHERE id = $4
         RETURNING *`,
        [engineerId, req.file.buffer, JSON.stringify(parsedFaceFeatures), attendanceId],
      );
...
router.post("/manual/checkout", engineerCheck, upload.single("faceImage"), async (req, res) => {
  try {
    const engineerId = req.user.id;
    ...
    if (attendanceRes.rows.length === 0) {
      return res.status(404).json({ error: "Manual attendance record not found" });
    }
    ...
    const access = await verifyEngineerAccess(engineerId, attendance.project_id);
    if (!access.allowed) return res.status(403).json({ error: access.error });

    // Parse face features
    let checkInFeatures, checkoutFeatures;
    try {
      checkInFeatures = typeof attendance.check_in_face_features === 'string'
        ? JSON.parse(attendance.check_in_face_features)
        : attendance.check_in_face_features;
      
      checkoutFeatures = typeof faceFeatures === 'string'
        ? JSON.parse(faceFeatures)
        : faceFeatures;
    } catch (e) {
      return res.status(400).json({ error: "Invalid face features JSON format" });
    ...
    console.log('[Face Verification] Check-in features:', JSON.stringify(checkInFeatures).substring(0, 200));
    console.log('[Face Verification] Checkout features:', JSON.stringify(checkoutFeatures).substring(0, 200));
    
    const verificationResult = compareFaceFeatures(checkInFeatures, checkoutFeatures);
...
function compareFaceFeatures(features1, features2) {
  ...
  if (!requiredKeys.every(k => keys1.includes(k)) || 
      !requiredKeys.every(k => keys2.includes(k))) {
    console.log('[Face Verification] Missing required landmarks');
    return { verified: false, confidence: 0, reason: 'Missing required landmarks' };
  }
  ...
  if (ratios1.eyeToNoseMouth != null && ratios2.eyeToNoseMouth != null) {
    ...
    if (match) matches++;
    ratioDiffs.push({ name: 'eyeToNoseMouth', diff, threshold, match, ratio1: ratios1.eyeToNoseMouth, ratio2: ratios2.eyeToNoseMouth });
  }

  // Ratio 2: Eye-Nose Distance / Nose-Mouth Distance
  if (ratios1.eyeNoseToNoseMouth != null && ratios2.eyeNoseToNoseMouth != null) {
    totalChecks++;
    const diff = Math.abs(ratios1.eyeNoseToNoseMouth - ratios2.eyeNoseToNoseMouth);
    const threshold = ratios1.eyeNoseToNoseMouth * tolerance;
    const match = diff < threshold;
    if (match) matches++;
    ratioDiffs.push({ name: 'eyeNoseToNoseMouth', diff, threshold, match, ratio1: ratios1.eyeNoseToNoseMouth, ratio2: ratios2.eyeNoseToNoseMouth });
  }
  ...
  if (ratios1.faceAspect != null && ratios2.faceAspect != null) {
    ...
    const match = diff < threshold;
    if (match) matches++;
    ratioDiffs.push({ name: 'faceAspect', diff, threshold, match, ratio1: ratios1.faceAspect, ratio2: ratios2.faceAspect });
  }
  ...
  if (ratios1.eyeToFaceWidth != null && ratios2.eyeToFaceWidth != null) {
    ...
    const match = diff < threshold;
    if (match) matches++;
    ratioDiffs.push({ name: 'eyeToFaceWidth', diff, threshold, match, ratio1: ratios1.eyeToFaceWidth, ratio2: ratios2.eyeToFaceWidth });
  }
  ...
  if (features1.eyeDistance > 0 && features2.eyeDistance > 0) {
    distanceChecks++;
    const eyeDistDiff = Math.abs(features1.eyeDistance - features2.eyeDistance) / features1.eyeDistance;
    if (eyeDistDiff < distanceTolerance) distanceMatches++;
  }

  if (features1.noseMouthDistance > 0 && features2.noseMouthDistance > 0) {
    distanceChecks++;
    const noseMouthDiff = Math.abs(features1.noseMouthDistance - features2.noseMouthDistance) / features1.noseMouthDistance;
  ...
  console.log('[Face Verification]', {
    ...
    ratioDiffs: ratioDiffs.map(r => `${r.name}: ${r.match ? '✓' : '✗'} (diff: ${r.diff.toFixed(4)}, threshold: ${r.threshold.toFixed(4)})`).join(', ')
  });
