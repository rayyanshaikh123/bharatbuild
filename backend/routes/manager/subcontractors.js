...
router.post("/", managerCheck, async (req, res) => {
  try {
    ...
    // Verify manager is approved in this organization
...
router.get("/:id", managerCheck, async (req, res) => {
  try {
    const managerId = req.user.id;
    const { id } = req.params;
    ...
    // Verify manager is approved in the organization
    const orgIds = await getManagerOrgIds(managerId);
...
router.get("/:id/performance", managerCheck, async (req, res) => {
  try {
    const managerId = req.user.id;
    const { id } = req.params;
    ...
    // Verify manager is in the organization
    const orgIds = await getManagerOrgIds(managerId);
    if (subcontractor.org_id && !orgIds.includes(subcontractor.org_id)) {
      return res
        .status(403)
