...
router.get("/engineer-requests", managerCheck, async (req, res) => {
  ...
  try {
    ...
    const isApproved = await managerprojStatusCheck(
      managerId,
      projectId,
    );
    if (!isApproved) {
...
router.put("/engineer-requests/:requestId/approve", managerCheck, async (req, res) => {
  try {
    const managerId = req.user.id;
...
router.put("/engineer-requests/:requestId/reject", managerCheck, async (req, res) => {
  try {
    const managerId = req.user.id;
    const { requestId } = req.params;
