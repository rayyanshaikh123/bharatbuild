...
const { ensureOwnerAuthenticated } = require("../../middleware/authMiddleware");

router.post("/create-organization", ensureOwnerAuthenticated, async (req, res) => {
...
router.get("/organizations", ensureOwnerAuthenticated, async (req, res) => {
    const ownerId = req.user.id;
    try {
        const result = await pool.query(
...
router.patch("/organization/:id", ensureOwnerAuthenticated, async (req, res) => {
...
router.delete("/organization/:id", ensureOwnerAuthenticated, async (req, res) => {
    const orgId = req.params.id;
    const ownerId = req.user.id;
    try {
        const result = await pool.query(
...
router.get("/organization/:id", ensureOwnerAuthenticated, async (req, res) => {
    const orgId = req.params.id;
    const ownerId = req.user.id;
    try {
        const result = await pool.query(
