...
router.get("/available", labourCheck, async (req, res) => {
    try {
        ...
        let query = `
      SELECT lr.*, p.name as project_name, p.location_text, p.latitude, p.longitude
    `;
        ...
        if (lat && lon) {
            query += `, 
            (6371000 * acos(cos(radians($1)) * cos(radians(p.latitude)) * cos(radians(p.longitude) - radians($2)) + sin(radians($1)) * sin(radians(p.latitude)))) AS distance_meters
            `;
            params.push(lat, lon);
            paramIdx = 3;
        }
