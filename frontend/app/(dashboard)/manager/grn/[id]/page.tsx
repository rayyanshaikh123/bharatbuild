...
import { Loader2, ArrowLeft, CheckCircle2, XCircle, FileText, Image as ImageIcon, AlertTriangle, AlertCircle } from "lucide-react";
...
export default function GRNVerificationPage() {
      ...
      try {
        ...
        if(res.grn) {
          ...
          receivedItems.forEach(recv => {
            const poItem = poItems.find(p => p.material_name === recv.material_name);
            if(!poItem) {
               results.push({ material_name: recv.material_name, status: "NOT_IN_PO" });
            } else {
               const poQty = parseFloat(poItem.quantity);
               const recvQty = recv.quantity_received;
               
               if(recvQty < poQty) {
                 results.push({ material_name: recv.material_name, status: "SHORT_RECEIVED", po_quantity: poQty, received_quantity: recvQty, difference: poQty - recvQty });
               } else if (recvQty > poQty) {
                 results.push({ material_name: recv.material_name, status: "OVER_RECEIVED", po_quantity: poQty, received_quantity: recvQty, difference: recvQty - poQty });
               } else {
                 results.push({ material_name: recv.material_name, status: "MATCHED", po_quantity: poQty, received_quantity: recvQty });
               }
            }
          });
        ...
        } catch(e) { console.error("Failed to load bill image"); }

        try {
            const proofBlob = await managerGrn.getDeliveryProofImage(id as string);
            setProofUrl(URL.createObjectURL(proofBlob));
        } catch(e) { console.error("Failed to load proof image"); }
  ...
  const handleAction = async (action: "APPROVE" | "REJECT") => {
    const remarks = window.prompt(action === "REJECT" ? "Reason for rejection (Required):" : "Approval remarks (Optional):");
      ...
      <div className="border-b border-border bg-background/50 backdrop-blur-md sticky top-0 z-10">
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
             <Badge variant={grn.status === "APPROVED" ? "default" : grn.status === "REJECTED" ? "destructive" : "secondary"}>
                {grn.status}
             </Badge>
          </div>
        </div>
      ...
      <div className="container mx-auto px-4 py-8 max-w-5xl space-y-8">
                ...
                <div className="glass-card rounded-2xl p-6">
                    ...
                    <div className="space-y-3">
                        {comparison.map((res, idx) => (
                            <div key={idx} className={`p-4 rounded-xl border flex items-center justify-between ${
                                res.status === "MATCHED" ? "bg-green-500/5 border-green-500/10" : 
                                res.status === "SHORT_RECEIVED" ? "bg-orange-500/5 border-orange-500/10" :
                                ...
                                <div>
                                    ...
                                    <div className="text-sm text-muted-foreground flex gap-4 mt-1">
                                        ...
                                        <span className="font-semibold text-foreground">Received: {res.received_quantity}</span>
                                    </div>
                                </div>
                                
                                {res.status === "MATCHED" && (
                                    <Badge className="bg-green-500/10 text-green-600 hover:bg-green-500/20 border-0">Matched</Badge>
                                )}
                 ...
                 <div className="glass-card rounded-2xl p-6 grid grid-cols-2 gap-4 text-sm">
                    ...
                    <div>
                        ...
                        <p className="font-medium">{new Date(grn.received_at).toLocaleDateString()} {new Date(grn.received_at).toLocaleTimeString()}</p>
                    </div>
                </div>
            </div>
                ...
                <div className="glass-card rounded-2xl p-6">
                    ...
                    <div className="grid p-1 gap-3">
                        <Button 
                            ...
                            disabled={!billUrl}
                        >
                            <FileText size={16} className="mr-2"/> View Bill
                        </Button>
                        <Button 
                            variant="outline" 
                            className="w-full justify-start h-12"
                            ...
                            disabled={!proofUrl}
                        >
                            <ImageIcon size={16} className="mr-2"/> View Delivery Proof
                        </Button>
                    </div>
                </div>
        ...
        {viewImage && (
            <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4 backdrop-blur-sm"
                ...
                <div className="relative max-w-4xl max-h-[90vh] w-full bg-background rounded-2xl overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200"
                     onClick={e => e.stopPropagation()}>
                    <div className="p-4 border-b flex justify-between items-center">
                        <h3 className="font-semibold">{viewImage === "BILL" ? "Vendor Bill" : "Delivery Proof"}</h3>
                        <Button variant="ghost" size="icon" onClick={() => setViewImage(null)}>
                            <XCircle size={20}/>
                        </Button>
                    </div>
