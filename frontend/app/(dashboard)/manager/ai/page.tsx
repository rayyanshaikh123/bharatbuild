...
import { Rocket, AlertTriangle, Package2, ShieldCheck, Activity, BrainCircuit } from "lucide-react";
...
function HealthAnalysis({ projectId }: { projectId: string }) {
  ...
  useEffect(() => {
    managerAI.getProjectHealth(projectId).then(setData).catch(console.error).finally(() => setLoading(false));
  }, [projectId]);
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
      ...
      <Card className="glass-card">
        ...
        <CardContent>
          <div className="text-3xl font-bold">{data.financial_health.utilization}%</div>
          <p className="text-sm text-muted-foreground mt-2">Utilization of budget</p>
          <p className="mt-2 text-sm">{data.financial_health.status}</p>
        </CardContent>
      </Card>
      ...
      <Card className="glass-card">
        ...
        <CardContent>
          <div className="text-3xl font-bold">{data.schedule_health.delayed_percentage}%</div>
          <p className="text-sm text-muted-foreground mt-2">Tasks delayed</p>
          <p className="mt-2 text-sm">{data.schedule_health.status}</p>
        </CardContent>
      </Card>
      ...
      <Card className="glass-card">
        ...
        <CardContent>
          <div className="text-3xl font-bold">{data.operational_health.score}/10</div>
          <p className="text-sm text-muted-foreground mt-2">Operational Score</p>
          <p className="mt-2 text-sm">{data.operational_health.issues}</p>
        </CardContent>
      </Card>
...
function DelayAnalysis({ projectId }: { projectId: string }) {
  ...
  useEffect(() => {
    managerAI.getDelayAnalysis(projectId).then(setData).catch(console.error).finally(() => setLoading(false));
  }, [projectId]);
    ...
    <div className="space-y-6">
      <Card className="glass-card">
        <CardHeader>
          ...
          <CardDescription>AI-identified primary causes for project delays</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            {data.root_causes?.map((cause: any, i: number) => (
              <div key={i} className="flex justify-between items-center border-b border-border pb-2 last:border-0">
                ...
                <span className="font-mono bg-muted px-2 py-1 rounded text-xs">{cause.probability}% Prob.</span>
              </div>
            ))}
          </div>
        </CardContent>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card className="glass-card">
           <CardHeader><CardTitle>Impact Assessment</CardTitle></CardHeader>
           <CardContent>
             <p className="text-sm text-muted-foreground">{data.impact_assessment}</p>
           </CardContent>
        </Card>
        <Card className="glass-card">
...
function MaterialAnalysis({ projectId }: { projectId: string }) {
  ...
  useEffect(() => {
    managerAI.getMaterialAnomalies(projectId).then(setData).catch(console.error).finally(() => setLoading(false));
  }, [projectId]);
    ...
    <div className="space-y-6">
       <Card className="glass-card">
        <CardHeader>
          ...
          <CardDescription>Detected irregularities in material usage vs requests</CardDescription>
        </CardHeader>
        <CardContent>
           {data.anomalies?.length === 0 ? (
             <p className="text-muted-foreground">No significant anomalies detected.</p>
           ) : (
             <div className="space-y-4">
...
function AuditAnalysis({ projectId }: { projectId: string }) {
  ...
  useEffect(() => {
    managerAI.getAuditInsights(projectId).then(setData).catch(console.error).finally(() => setLoading(false));
  }, [projectId]);
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2">
      <Card className="glass-card">
       ...
       <Card className="glass-card">
        <CardHeader><CardTitle>Unusual Patterns</CardTitle></CardHeader>
        <CardContent>
...
export default function ManagerAIPage() {
  ...
  return (
    <div className="space-y-8 pt-12 md:pt-0 pb-12">
      <DashboardHeader userName={user?.name?.split(" ")[0]} title="AI Insights Center" />

      {/* Controls */}
      <div className="glass-card rounded-2xl p-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
           <BrainCircuit className="text-primary h-5 w-5" />
           <span className="font-medium hidden md:block">Select project to analyze</span>
        </div>
        ...
        {!selectedProjectId ? (
          ...
          <Tabs defaultValue="health" className="space-y-6">
            <TabsList className="bg-background/20 backdrop-blur border border-border">
              ...
              <TabsTrigger value="delays" className="data-[state=active]:bg-primary/20">
            ...
            <TabsContent value="health" className="animate-in fade-in-50 slide-in-from-bottom-2">
              <HealthAnalysis projectId={selectedProjectId} />
            </TabsContent>
            
            <TabsContent value="delays" className="animate-in fade-in-50 slide-in-from-bottom-2">
              <DelayAnalysis projectId={selectedProjectId} />
            </TabsContent>

            <TabsContent value="materials" className="animate-in fade-in-50 slide-in-from-bottom-2">
              <MaterialAnalysis projectId={selectedProjectId} />
            </TabsContent>

            <TabsContent value="audit" className="animate-in fade-in-50 slide-in-from-bottom-2">
