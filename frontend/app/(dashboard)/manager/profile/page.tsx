...
import { managerProfile, managerLeaveOrganization, managerOrganization } from "@/lib/api/manager";
...
export default function ManagerProfilePage() {
    ...
    const fetchData = async () => {
      try {
        ...
        const [profileRes, orgRes] = await Promise.all([
          managerProfile.get(),
          managerOrganization.getMyOrganizations()
        ]);
        
        setProfile(profileRes.manager);
  ...
  const handleLeaveOrganization = async () => {
    try {
      setIsLeaving(true);
      await managerLeaveOrganization.leave();
      toast.success("You have left the organization");
      router.push("/manager"); // Redirect to dashboard or home
    } catch (err: any) {
  ...
  return (
    <div className="space-y-8 pt-12 md:pt-0 max-w-2xl mx-auto">
      {/* Header */}
      ...
      <div className="bg-card border border-border rounded-2xl p-8 space-y-6">
         <div className="flex items-center gap-6">
            ...
            <div>
               <h2 className="text-2xl font-bold text-foreground">{profile?.name || "Manager"}</h2>
                          ...
                          <DialogHeader>
                            ...
                            <DialogDescription>
                              ...
                              <ul className="list-disc pl-5 mt-2 space-y-1">
                                ...
                                <li>Require you to join a new organization to continue working</li>
                              </ul>
                            </DialogDescription>
                          </DialogHeader>
                          <DialogFooter>
