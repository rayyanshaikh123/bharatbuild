...
// ==================== MANAGER QA ENGINEER REQUESTS API ====================
...
export interface ProjectQAEngineer {
  id: string;
  project_id: string;
  qa_engineer_id: string;
  status: string;
...
export const managerQAEngineerRequests = {
  // Get pending requests for manager's organization
  getOrgPending: () =>
    api.get<{ pending_engineers: QAEngineerRequest[] }>("/manager/qa-engineer-requests/organization-pending"),

  // Approve/Reject organization request
  updateOrgStatus: (requestId: string, action: "APPROVE" | "REJECT") =>
    api.patch<{ request: QAEngineerRequest }>(`/manager/qa-engineer-requests/organization/${requestId}`, { action }),

  // Assign QA Engineer to Project
  assignToProject: (projectId: string, qaEngineerId: string) =>
    api.post<{ assignment: ProjectQAEngineer }>(`/manager/qa-engineer-requests/project/${projectId}/assign`, {
      qa_engineer_id: qaEngineerId,
    }),

  // Get QA Engineers assigned to a project
  getProjectEngineers: (projectId: string) =>
    api.get<{ qa_engineers: ProjectQAEngineer[] }>(`/manager/qa-engineer-requests/project/${projectId}`),

  // Remove QA Engineer from project
  removeFromProject: (projectId: string, qaEngineerId: string) =>
    api.delete<{ message: string }>(`/manager/qa-engineer-requests/project/${projectId}/${qaEngineerId}`),
};
